...
class RDNetClassifierHead(nn.Module):
    '''
    final layer for classification, fairly standard
    '''
    def __init__(
        self,
        in_features: int,
...
class PatchifyStem(nn.Module):
    '''
    initial CNN feature map learner
    fairly common in all CNNs
    uses a stride to avoid overlearning initial features
    layernorm is individual batch element normalization, uses learned beta/gamma params for better normalization 
    '''
...
class Block(nn.Module):
    ...
    '''
    block that makes up one section of the densestage
    run a large kernel conv for compression
    layernorm
    another conv w/ kernel 1 for learning across channels
    that convolve is effectively creating "word embeddings"
    but instead, it's inter_chs vector len features
    inter_chs = 16
    out_chs = 64
    so, 32x32x3 [RGB] => 32x32x16 [len 16 features] => non-linear [sparser features] => 32x32x64 [len 64 features]
    '''
...
class DenseBlock(nn.Module):
    ...
    drop path for regularization, like dropout but it picks entire "blocks" (so the layers above) instead of neurons
    growth rate is the total out_chs size
    "bottleneck layer" is that inter_chs layer pre growth rate
    '''
    def __init__(
        self,
        num_input_features,
...
class DenseStage(nn.Sequential):
    '''
    stage is the num of blocks (usually 3 w/ some droppath chance)
    we concat the learned features each step through the densestages
    this is where transition layers come into play
    we have learned compressions to cull some of the large growth rate feature sizes
    '''
...
'''
all the config code for hugging face hosting
...
num_init_features:
- num of features to learn in the first layer

growth_rates:
- growth rate for concats
- only grabs this many layers
- some type of learned feature compression

num_blocks_list
- num of blocks
- usually 3 blocks per stage

is_downsample_block
- do we downsample? unsure at which stage
- assuming prior to stages

transition_compression_ratio
- transition layers compression
- concats grow quickly => we usually only take half each time

block_type
- regular block or squeeze-excitation block
- covered in the senet paper (https://arxiv.org/pdf/1709.01507)
gpt explaination:
squeeze:
take the spatial information in each channel (the feature map) and reduce it to a single scalar per channel. this scalar represents a distilled, global summary of the channel's "importance."
excitation:
use the squeezed scalar values as context and pass them through a learnable mechanism (typically a small multi-layer perceptron). this generates a set of weights (one per channel) that determine how much to amplify or suppress each channel.
recalibration:
scale the original feature maps for each channel using the learned weights. this focuses the model's attention on the most useful channels while reducing the influence of less relevant ones.
- we do not implement this in our rewrite
'''
